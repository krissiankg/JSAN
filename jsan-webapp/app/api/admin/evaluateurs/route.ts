...
export async function GET() {
  ...
  try {
    ...
    const evaluators: EvaluatorRow[] = ((profiles ?? []) as Omit<EvaluatorRow, 'email'>[]).map((profile) => {
    ...
    return NextResponse.json({ ok: true, evaluators });
  } catch (err) {
    return NextResponse.json(
      { ok: false, error: err instanceof Error ? err.message : 'Impossible de charger les évaluateurs.' },
      { status: 500 }
    );
  }
}
